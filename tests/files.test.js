...
describe('Files Controller', () => {
    ...
    it('should return 201 and file data when file upload is successful', async () => {
        ...
        const result = await filesService.create({ user: req.user.userId, file: req.file });
    ...
    it('should return 500 when file upload fails', async () => {
        ...
        expect(validateFile).toHaveBeenCalledWith(req.file, res, ['application/pdf']);
        expect(filesService.create).toHaveBeenCalledWith({ user: req.user.userId, file: req.file });
